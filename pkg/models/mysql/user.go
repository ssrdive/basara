...
func (m *UserModel) Get(username, password string) (*models.JWTUser, error) {
	...
	err := m.DB.QueryRow("SELECT id, username, password, name, type, warehouse_id FROM user WHERE username = ?", username).Scan(&u.ID, &u.Username, &u.Password, &u.Name, &u.Type, &u.WarehouseID)
	if err != nil {
